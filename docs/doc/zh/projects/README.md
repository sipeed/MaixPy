...
这里汇总三类内容：可以直接安装的应用、可以跟着制作的项目，以及公开源码的工具和项目。应用不一定公开源码，是否提供源码请以应用详情页为准。
...
下面既有设备内置或可安装的应用，也有社区项目和开源工具。它们提供的资料不同：有的只有使用说明，有的还会提供源码、制作文档或演示视频。

### 内置应用

下面是 `MaixCAM`、`MaixCAM Pro` 和 `MaixCAM2` 等设备自带或常用的应用。列在这里表示可以查看和使用，不代表一定公开源码。
