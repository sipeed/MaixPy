...
本文面向会使用 Python、Docker，并能通过 Netron 查看 ONNX 输入输出的读者，本流程只以 YOLOv8 检测模型为完整示例
...
上表中的节点名来自常见 Ultralytics 导出版本，只能帮助定位，不能代替 shape 检查。模型版本变化时层编号可能不同，请用 Netron 找到位置和含义相同的节点。

对于本文的 YOLOv8 Detect 的输出方案，输入为 `H×W` 时，候选框数为：

```text
N = (H/8 × W/8) + (H/16 × W/16) + (H/32 × W/32)
```

例如输入为 `320×224`，则 `N = 1470`，80 类模型的两个输出应分别表示 4 个边框坐标和 80 个类别分数，所以 shape 为 `[1,4,1470]` 和 `[1,80,1470]`。
4 个边框坐标和 80 个类别分数：80 是使用的 COCO 基准模型有 80 个类别，所以分类输出的通道数是 80，自定义模型应改为自己的类别数 `C`。4 是每个候选位置的四个边框距离回归量，分别是左、上、右、下，即 left、top、right、bottom，它们是 DFL 处理后的距离。即一个框需要预测上、下、左、右四个方向的距离。

若节点名称相似但 shape、输出数量或语义不符，不要继续转换。
