...
目标检测指的是“找出物体并标出位置”。分类、姿态、分割、旋转框和旧版 YOLOv5 暂不支持，需要使用页面末尾的手动转换入口。
...
这些图片只帮助转换工具适配板子，不会重新训练模型，所以只放图片即可，不用准备标注文件。
...
网页不支持时，再看[MaixCAM 手动转换](./maixcam.md)、[MaixCAM2 手动转换](./maixcam2.md)或[移植新模型](../pro/customize_model.md)。这些进阶路线需要安装专用转换环境，并了解模型的输入和输出。
