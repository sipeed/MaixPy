...
- **想让板子识别自己的物品，希望操作尽量简单**：用 [MaixHub 在线训练](../vision/maixhub_train.md)。网页会带你完成拍照、标注和训练，推荐新手选择这条路线。
...
网页转换不支持的模型才需要进入进阶文档：[手动转换给 MaixCAM](./maixcam.md)、[手动转换给 MaixCAM2](./maixcam2.md)或[移植 MaixPy 尚未支持的模型](../pro/customize_model.md)。
