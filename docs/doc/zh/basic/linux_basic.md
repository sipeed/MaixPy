...
第一次运行 MaixPy 例程时，可以先跳过本页。需要使用终端、传输文件或排查系统问题时再回来阅读。
...
你可以先把 Linux 理解为管理设备硬件、文件和程序的操作系统。它给 MaixCAM 带来几个直接好处：
...
这里介绍最常用的网络传输方式。日常开发也可以直接使用 MaixVision 的设备文件管理器。
...
* 电脑可以通过 SCP 或 SFTP 把文件传到设备，常用工具包括：
...
本页后面列出了 MaixCAM 常用命令。需要完整学习时，可以继续查阅一套 Linux 命令行教程；日常使用不需要一次记住所有命令。
