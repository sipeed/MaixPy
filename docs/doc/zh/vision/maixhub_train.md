...
这篇以“让板子识别苹果”为例。图片采集、画框和训练都在 MaixHub 完成，不需要在电脑上安装训练工具。
...
项目类型选择“图像检测”。检测模型会告诉你苹果在哪里，并在画面上画框。如果只选“图像分类”，模型只能判断整张图是不是苹果，不能给出位置。
...
打开“标注数据”页面，用矩形贴着苹果边缘画框，然后选择 `apple`。
