...
上面提供的是 COCO 数据集的 80 类模型。如果不满足需求，可以先参考[在电脑训练 YOLO 检测模型](./customize_model_yolo.md)了解图片准备、标注和训练流程，再按 YOLOv8/YOLO11 官方的分割任务格式训练。分割模型暂不支持网页转换，需要使用对应板子的手动转换文档。
