...
带屏幕版本可以直接操作内置应用、修改常用设置并查看实时画面，更适合初次开发。无屏幕版本通常用于成本敏感或批量部署的项目，需要通过电脑完成更多设置。

如果你使用的是 MaixCAM Lite 或其他无屏幕版本，请继续阅读本页。
...
先使用质量可靠的 USB 线，并确认电脑 USB 口至少能提供 `5V 500mA`。首次启动等待约 20 秒，电脑的网络设置中应出现一个或两个 USB 虚拟网卡。

如果没有出现虚拟网卡：
* 请确认购买了配套的 TF 卡，如果确认有 TF 卡，并且已经插入到设备，可以**尝试[更新到最新的系统](./basic/os.md)**。
* 如果你没有购买 TF 卡套餐，你需要按照[升级和烧录系统](./basic/os.md)的方法烧录最新的系统到 TF 卡。
* 检查 USB 接头是否松动，并换一根可靠的数据线测试。
* 换一个能稳定供电的 USB 口，必要时换一台电脑测试。
...
运行库提供应用需要的基础功能。没有安装最新版时，程序可能无法运行或突然退出。
