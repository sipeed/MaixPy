...
没有板载 IMU 时，可以外接 MPU6050、MPU9150 或其他兼容传感器。选型时确认供电电压、通信接口和量程满足项目需求。
...
第一次运行时需要调用 `calib_gyro` 校准陀螺仪。设备静止时，程序连续采样并计算平均偏差；`calib_gyro(10000)`会采样 10 秒，并把结果保存到 `/maixapp/share/misc`。
...
加速度计也可能需要校准，但具体方法取决于传感器和精度要求。本例不依赖加速度校准。

### 磁力计校准

磁力计通常需要校准软铁和硬铁误差，常见方法是椭球拟合。板载传感器没有磁力计，因此本例不展开这一步。
...
`PI` 控制器的两个参数会影响响应速度和稳定性。先使用默认值，再根据实际抖动和响应速度小幅调整：
