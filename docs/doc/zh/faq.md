...
不建议直接更换。接口、电压、驱动或初始化参数不匹配时，设备可能无法工作，甚至损坏硬件。

官方配件已经完成硬件和驱动适配。使用其他摄像头或屏幕前，请先核对接口、电压、引脚、驱动和初始化时序。完成新硬件适配后，欢迎提交 PR。
...
* 如果下载页面提供 SHA-256 或 MD5 值，请在电脑上计算文件校验值并对比。Linux 和 macOS 可以运行 `sha256sum 文件名`，Windows PowerShell 可以运行 `Get-FileHash 文件名`。
* 如果是来自压缩包，请确认解压过程没有出错，可以从压缩包重新解压一遍保证中间没有出错。
* 为了确认传输过程没有损坏文件，分别在电脑和设备上计算 SHA-256；两个结果应该完全相同。
...
请按 [MaixVision 使用文档](./basic/maixvision.md)中的“打开文件夹”部分组织和运行多文件项目。
...
默认只有英文字体。如果要显示中文，需要加载中文字体，具体请看[基本图像操作中的中文支持和自定义字体](./vision/image_ops.md#中文支持和自定义字体)。
