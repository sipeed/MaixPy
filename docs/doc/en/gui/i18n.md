...
The scanning tool finds strings passed to `tr()` and collects them in translation files. This makes it less likely that a new interface string will be left untranslated.
...
* Each generated file contains key-value pairs. For example, `zh.yaml` initially contains `hello: hello`; change the value on the right to Chinese: `hello: 你好`. Use a UTF-8 editor such as MaixVision or VS Code. On Windows, do not save the file with GBK encoding.
