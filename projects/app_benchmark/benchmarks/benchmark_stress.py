...
class Bechmark:
    ...
    def stress_process(self, model_path, nee_exit, model_fps):
        ...
        img = image.load("/maixapp/share/picture/2024.1.1/test_coco_640.jpg").resize(self.detector.input_width(), self.detector.input_height(), image.Fit.FIT_COVER)
    ...
    def run(self):
        ...
        touch_pressed = False
        cam_res_curr_idx = 0
        record_path = f"/root/benchmark_stress_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.txt"
        ...
        while (not self.need_exit) and (not app.need_exit()):
            if time.ticks_s() - self.last_read_cpu_temp > self.read_interval:
                ...
                self.fps_values.append(model_fps.value)
                cpu_usage = sys.cpu_usage()["cpu"]
                npu_usage = sys.npu_usage()["npu"]
                cpu_freq = sys.cpu_freq()["cpu0"]
                npu_freq = sys.npu_freq()["npu0"]
                f.write(f"{time.time()}, {cpu_temp}, {cpu_usage}, {npu_usage}\n")
                f.flush()
            img = self.cam.read()
            y = self.img_back.height() + self.button_padding_2x
            img.draw_string(2, y, f"[CPU] usage: {cpu_usage:3.1f}%, temp: {cpu_temp:3.1f} C, freq: {cpu_freq // 1000000}MHz", image.COLOR_WHITE, scale=self.font_scale, thickness=4)
            img.draw_string(2, y, f"[CPU] usage: {cpu_usage:3.1f}%, temp: {cpu_temp:3.1f} C, freq: {cpu_freq // 1000000}MHz", image.COLOR_RED, scale=self.font_scale, thickness=2)
            y += self.font_size.height() + self.font_margin
            img.draw_string(2, y, f"[NPU] usage: {npu_usage:3.1f}%, ai_isp_on: {self.ai_isp_on}, freq: {npu_freq // 1000000}MHz", image.COLOR_WHITE, scale=self.font_scale, thickness=4)
            img.draw_string(2, y, f"[NPU] usage: {npu_usage:3.1f}%, ai_isp_on: {self.ai_isp_on}, freq: {npu_freq // 1000000}MHz", image.COLOR_RED, scale=self.font_scale, thickness=2)
            y += self.font_size.height() + self.font_margin
            img.draw_string(2, y, f"model FPS: {model_fps.value:3d}, model: {model_name}", image.COLOR_WHITE, scale=self.font_scale, thickness=4)
            img.draw_string(2, y, f"model FPS: {model_fps.value:3d}, model: {model_name}", image.COLOR_PURPLE, scale=self.font_scale, thickness=2)
            y += self.font_size.height() + self.font_margin
            fps = time.fps()
            img.draw_string(2, y, f"[CAM] {self.cam.width()}x{self.cam.height()}, fps: {self.cam.fps()}, {fps:.0f}", image.COLOR_WHITE, scale=self.font_scale, thickness=4)
            img.draw_string(2, y, f"[CAM] {self.cam.width()}x{self.cam.height()}, fps: {self.cam.fps()}, {fps:.0f}", image.COLOR_RED, scale=self.font_scale, thickness=2)
            ...
            img.draw_line(0, img.height() - y, img.width(), img.height() - y, image.COLOR_WHITE, thickness=1)
            img.draw_string(0, img.height() - y + 2, "70", image.COLOR_YELLOW, scale=self.font_scale, thickness=1)
            ...
            img.draw_line(0, img.height() - y, img.width(), img.height() - y, image.COLOR_WHITE, thickness=1)
            img.draw_string(0, img.height() - y + 2, "50", image.COLOR_YELLOW, scale=self.font_scale, thickness=1)
